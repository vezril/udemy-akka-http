...
import part2lowlevelserver.GuitarDB.{ CreateGuitar, FindAllGuitars }
...
class GuitarDB extends Actor with ActorLogging {
  ...
  override def receive: Receive = {
    ...
    case FindGuitar(id) => {
      log.info(s"Searcing guitar by id: $id")
...
object LowLevelRest extends App with GuitarStoreJsonProtocol {
  ...
  * - POST to /api/guitar/inventory?id=X&quantity=Y which adds Y guitar to the stock for guitar with id X
  */
