...
object HttpsContext {
  ...
  // alternative: new FileInputStream(new File("src/main/ressources/keystore.pkcs12))"
...
object LowLevelHttps extends App {
  ...
  val httpsBinding =
    Http().bindAndHandleSync(requestHandler, "localhost", 8443, connectionContext = HttpsContext.httpsConnectionContext)
