...
class RouteDSLSpec extends WordSpec with Matchers with ScalatestRouteTest with BookJsonProtocol {
  ...
  "A digital library backend" should {
    ...
    "should insert a book into the database" in {
...
object RouteDSLSpec extends BookJsonProtocol with SprayJsonSupport {
  ...
  /*
    ...
    GET /api/book/author/X - return all the books from the author X
   */
  val libraryRoute =
    pathPrefix("api" / "book") {
      (path("author" / Segment) & get) { author => complete(books.filter(_.author == author))
      } ~
        get {
          (path(IntNumber) | parameter('id.as[Int])) { id => complete(books.find(_.id == id))
          } ~
